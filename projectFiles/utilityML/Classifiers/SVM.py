...
class SVM_linear:
	...
	def compute_dcf(self, threshold = None):
		confusion_matrix = compute_confusion_matrix_binary(self.LTE, self.score[0], self.priors[1], 1, 1, threshold)
		return compute_normalized_dcf_binary(confusion_matrix, self.priors[1], 1, 1)

	def compute_min_dcf(self):
		return compute_min_dcf(self.LTE, self.score[0], self.priors[1], 1, 1)
...
class SVM_poly:
	...
	def compute_dcf(self, threshold = None):
		confusion_matrix = compute_confusion_matrix_binary(self.LTE, self.score, self.priors[1], 1, 1, threshold)
		return compute_normalized_dcf_binary(confusion_matrix, self.priors[1], 1, 1)

	def compute_min_dcf(self):
		return compute_min_dcf(self.LTE, self.score, self.priors[1], 1, 1)
